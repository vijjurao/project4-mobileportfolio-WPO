...
*/
/**
 * @fileOverview In this project I made Cam's Pizzeria website run at 60 frames
 * per second. I measured before I made optimizations to achieve higher performance.
 */
...
var pizzaElementGenerator = function(i) {
  ...
  pizzaImage.src = "dist/png/pizza-165_small.png";
...
var resizePizzas = function(size) {
  ...
  changeSliderLabel(size);
   ...
   //Time to resize pizzas with improved for loop: ~ 0.5349999999998545ms

    /**
     * Access the DOM with the more effecient getElementsByClassName method 
     * then store it as a variable. Then used this variable inside a for loop
     * to change pizza sizes based on percent width with a slider.
  ...
  function changePizzaSlices(size) {
    // Changes the slider value to a percent width
     ...
     // Change querySelectorAll to more effecient getElementsByClassName

    var randomPizzaContainer = document.getElementsByClassName("randomPizzaContainer");
    //Access the DOM and store it as a variable. Use this variable inside the for loop
    var lengthRandomPizzaContainer = randomPizzaContainer.length;
    for (var i = 0; i < lengthRandomPizzaContainer; i++) {
       //Add '%' to new calculated Pizza Container width
        randomPizzaContainer[i].style.width = newWidth + "%";
    }
  }
  changePizzaSlices(size);
...
//Reduced amount of randomPizzas to be created and appended to the page
//Declare pizzasDiv variable outside of the loop
//to make only one DOM call
...
/**
 * Animates the pizzas in the background when scrolling.<br>
 *
 * Details at {@link http://www.html5rocks.com/en/tutorials/speed/animations/}<br>
 * Decouples scroll event from animation.
 * Then only uses requestAnimationFrame when there are visual updates.
 */

//Implemented requestAnimationFrame that runs only when scrolling
//by keeping track of the last known scroll y value and using it as
...
function updatePositions() {
    ...
    //the last known scroll y value is now the current
    //scroll y value that is used by rAF
    var currentScrollY = latestKnownScrollY;


/**
     * Cached the scrollTop value to be used inside a for loop
     * reducing the amount of times the DOM being accessed
     * @member {number} cachedScrolling
     */
    var cachedScrolling = document.body.scrollTop;

    /**
     * Replaced querySelectorAll with much more effecient getElementsByClassName
     * @member {string} items
     */
    var items = document.getElementsByClassName('mover');

    /**
     * Empty array that will store the pushed mover nodes
     * @member {Array} itemArray
     */
    var itemArray = [];

    //array of items to loop through
    //Cache items.length in a new variable that will be used
    //to loop through. Pushes each item into the empty array
    ...
    /**
     * i % 5 will return a remainder numbers 0 - 4 no
     * matter how large the number. Stored these values
     * in an array that will be incremented
     * @member {Array} phaseArray
     */
    ...
    for (var i = 0; i < lengthItemArray; i++) {
         ...
         * @member {number} itemArray[i]]
         */
        itemArray[i].style.transform = "translateX(" + basicMoveLeft + "px)";
    }
...
//Variables to be used with requestAnimationFrame
var latestKnownScrollY = 0,
    ticking = false;

/**
 * Stores the window's scroll position
 */
function onScroll() {
...
/**
 * calls requestAnimationFrame if one isn't already requested
 */
...
/**
 * @event
 * runs onScroll when page is being scrolled
 */
...
/**
 * @event
 * Once page loads the pizza images in the background are created.
 */
document.addEventListener('DOMContentLoaded', function() {
    ...
    /**
     * Used cloneNode method to duplicate images of pizzas then appends them
     * after the DOM content has loaded
     * @member {Boolean} elem_prime
     */
    var elem = document.createElement('img');

    /**
     * Called when DOM loads and has reduced amount of pizzas created in background to only what is visible above the fold
